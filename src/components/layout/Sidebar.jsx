...
function SidebarBtn({ id, label, badge, dot, icon, active, onClick }) {
  ...
  return (
    ...
    >
      ...
      {badge > 0 && (
        <span style={{ fontSize: 10, color: on ? C.accent : C.text2, fontVariantNumeric: 'tabular-nums' }}>
          {badge}
        </span>
      )}
    </button>
...
export default function Sidebar({ active, setActive, tickets, role }) {
  const qc = {};
  QUEUES.forEach((q) => {
    qc[q.id] = tickets.filter(
      (t) => t.queue === q.id && !['closed', 'resolved'].includes(t.status)
    ).length;
  });

  const ROLE_OWNERS = { agent: 'Camilo Reyes', admin: 'Patricia Lara', customer: 'María García' };
  const myCount  = tickets.filter((t) => t.assignee === ROLE_OWNERS[role] && !['closed', 'resolved'].includes(t.status)).length;
  const unCount  = tickets.filter((t) => t.assignee === 'Sin asignar' && !['closed', 'resolved'].includes(t.status)).length;
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 4px' }}>
        ...
        {role !== 'customer' && (
          <Section label="Bandejas">
            {QUEUES.map((q) => (
              <SidebarBtn key={q.id} id={`q_${q.id}`} label={q.name} badge={qc[q.id]} dot={q.color} active={active} onClick={setActive} />
            ))}
          </Section>
        )}
        ...
        <Section label="Herramientas">
          {toolItems.map((n) => (
            <SidebarBtn key={n.id} id={n.id} label={n.label} active={active} onClick={setActive} />
          ))}
        </Section>
