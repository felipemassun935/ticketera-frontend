...
export default function HistoryEntry({ entry, isLast }) {
  ...
  const fromQ = queues.find(q => q.id === entry.from_val);
  const toQ   = queues.find(q => q.id === entry.to_val);

  return (
    <div style={{ display: 'flex', gap: 12, paddingBottom: isLast ? 0 : 18, position: 'relative' }}>
      {!isLast && <div style={{ position: 'absolute', left: 11, top: 26, bottom: 0, width: 1, background: 'var(--border)' }} />}

      <div style={{ width: 23, height: 23, borderRadius: '50%', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: 'var(--bg2)' }}>
      ...
      <div style={{ flex: 1, paddingTop: 2, minWidth: 0 }}>
        ...
        {entry.type === 'status_change' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: entry.comment ? 5 : 0 }}>
            <StatusBadge status={entry.from_val || 'new'} />
            <Icon d={ARROW} size={11} varColor="--text2" />
            <StatusBadge status={entry.to_val} />
          </div>
        )}
        ...
        {entry.type === 'queue_move' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: entry.comment ? 5 : 0 }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: C.text1 }}>
              <QDot qid={entry.from_val} />{fromQ ? fromQ.name : entry.from_val}
            </span>
            <Icon d={ARROW} size={11} varColor="--text2" />
            <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--teal)', fontWeight: 500 }}>
              <QDot qid={entry.to_val} />{toQ ? toQ.name : entry.to_val}
            </span>
          </div>
        )}
