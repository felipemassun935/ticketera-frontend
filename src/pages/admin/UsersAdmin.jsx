...
const EMPTY = { name: '', email: '', role: 'agent', active: true };
...
function RolePill({ role, roles }) {
  ...
  return (
    <span style={{
      ...
      color: r?.color || C.text2,
      background: (r?.color || '#888') + '18',
...
export default function UsersAdmin() {
  ...
  function openEdit(u) { setForm(u);     setModal('edit'); }
  ...
  let rows = [...users];
  if (roleF !== 'all') rows = rows.filter(u => u.role === roleF);
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '22px' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 1 }}>
          {['all', ...roles.map(r => r.id)].map(rid => {
            ...
            return (
              <button key={rid} onClick={() => setRoleF(rid)} style={{
                padding: '3px 9px', borderRadius: 3, border: 'none', fontSize: 10, fontWeight: 500,
                background: roleF === rid ? C.accentMuted : 'transparent',
                color: roleF === rid ? C.accent : C.text2, cursor: 'pointer',
              }}>
      ...
      <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 6, overflow: 'hidden' }}>
        ...
        {rows.length === 0 && (
          <div style={{ padding: '24px', textAlign: 'center', fontSize: 12, color: C.text2 }}>Sin resultados</div>
        )}
        ...
        {rows.map((u, i) => (
          <div key={u.id} style={{
            ...
            opacity: u.active ? 1 : 0.45, transition: 'opacity 0.2s',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <RolePill role={u.role} roles={roles} />
            <Toggle on={u.active} onToggle={() => toggleUser(u.id)} />
            <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
              {confirm === u.id ? (
                <ConfirmDelete name={u.name} onConfirm={() => { removeUser(u.id); setConfirm(null); }} onCancel={() => setConfirm(null)} />
              ) : (
                <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {modal === 'add' && (
              <FormField label="Contraseña inicial">
                ...
                <div style={{ fontSize: 10, color: C.text2, marginTop: 4 }}>El usuario deberá cambiarla al ingresar.</div>
              </FormField>
            )}
            <FormField label="Rol">
              <select value={form.role} onChange={e => f('role', e.target.value)} style={{ ...iS, width: '100%' }}>
